		...
		create table full_gss as 
			select * from connection to odbc(
				select 
					...
					,case when cast(gss.[gss_pq_dvage_code] as smallint) between 15 and 25 then '1. 15 to 25'
...
/*						+coalesce(part.[P_INC_INC_INC_frdtwi_CST],0)*/
				...
				where gss.[gss_pq_dvage_code] < 65
...
%si_write_to_db(si_write_table_in=data_gss,
	si_write_table_out=&si_sandpit_libname..of_gss_desc_stats_tab_mh
...
%macro xlsx(name);
proc export data=&name. outfile="&si_source_path.\output\desc_stats_refresh_nofmt.xlsx" dbms=xlsx replace; 
sheet="&name."; run;
%mend;


%descstats_generator(indata = data_gss, varlist = &varlist., class =  admin_benefit_flag, outdata = descstats_by_benflag);
...
%descstats_generator(indata = data_gss, varlist = &varlist., class =  %str(age sex), outdata = age_sex_benefit );
proc export data=age_sex_benefit outfile="&si_source_path.\output\desc_stats_refresh_agesex.xlsx" dbms=xlsx replace; 
