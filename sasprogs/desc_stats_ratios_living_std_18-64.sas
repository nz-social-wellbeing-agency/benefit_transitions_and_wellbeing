...
/*		MWI low (<9_*/
		case when econ_material_well_being_idx2 < 8 then 1 else 0 end as MWI_low
	from [IDI_Sandpit].[DL-MAA2016-15].[of_gss_desc_stats_tab_mh18p] where gss_wave in ('GSS2014','GSS2016');
...
%let varlist = %str(econ_vege_bin econ_dr_bin econ_shop_bin econ_hobby_bin econ_cold_bin econ_appliance_bin econ_shoes_bin
                 ...
                 subj_life_satisfaction_low subj_sense_of_purpose_low MWI_low);
...
proc sql;
	...
	coalesce(econ_vege_bin, econ_dr_bin, econ_shop_bin, econ_hobby_bin, econ_cold_bin, econ_appliance_bin, econ_shoes_bin,
                 ...
                 house_crowding_bin, house_mold, house_cold, multiple_hardship,  culture_discrimination_bin,  safety_crime_victim, safety_neighbourhood_night_bin,
                 health_depressed_bin, health_pain_bin, civic_generalised_trust_bin, culture_identity_bin, social_time_lonely_bin,
                 subj_life_satisfaction_low, subj_sense_of_purpose_low, MWI_low) as var_value
	from output_cross;
quit;
...
proc export data=WORK.risks_output_bi
			outfile="&si_source_path./output/living_std_risk_ratios_18-64.xlsx" 
			dbms=xlsx replace; 	
			sheet="2014_bivar"; 
run;
proc export data=risks_output outfile="&si_source_path./output/living_std_risk_ratios_18-64.xlsx" dbms=xlsx replace; 	sheet="2014_trivar"; run;

/*2016 only */
%risk_ratios(ds=_temp_gss20146_stats2(where=(gss_wave='GSS2016')));
proc export data=risks_output_bi outfile="&si_source_path./output/living_std_risk_ratios_18-64.xlsx" dbms=xlsx replace; 	sheet="2016_bivar"; run;
proc export data=risks_output outfile="&si_source_path./output/living_std_risk_ratios_18-64.xlsx" dbms=xlsx replace; 	sheet="2016_trivar"; run;
