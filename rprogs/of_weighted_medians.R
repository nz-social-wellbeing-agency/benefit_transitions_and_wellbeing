...
gss_person <- gss_person %>% filter(snz_spine_ind_partner==1)
...
bivariate_means <- function(inputVar, inputVar2, measure){
  ...
  tmp1 <- gss_person_final_svy %>%
    ...
    group_by(var, var2) %>%
    summarise(
      wtmean = survey_median(meas, na.rm = TRUE, vartype = NULL)
...
univariate_means <- function(inputVar, measure, weighted = TRUE){
  ...
  if(weighted){
    tmp <- gss_person_final_svy %>%
      ...
      group_by(var) %>%
      summarise(wtmean = survey_median(meas, na.rm = TRUE, vartype = NULL)
                ,wttotal = survey_total(na.rm = TRUE, vartype = c("ci", "se"))
                ,unwttotal = unweighted(n())
      ) %>%
      mutate(var_name = rlang::quo_text(enquo_input)
...
univar_agg <-rbind(univariate_list$GSS2016%>%dplyr::select(var, var_name,measure_name, wtmean_q50, unwttotal, wttotal)
                   %>%mutate(wave="GSS2016"),
       univariate_list$GSS2014%>%dplyr::select(var, var_name, wtmean_q50,measure_name, unwttotal, wttotal)
                  %>%mutate(wave="GSS2014")
       )



bivar_agg <-rbind(bivariate_list$GSS2016%>%dplyr::select(var, var_name, var2, var_name2,measure_name, wtmean_q50, unwttotal, wttotal)
                   %>%mutate(wave="GSS2016"),
                   bivariate_list$GSS2014%>%dplyr::select(var, var_name, var2, var_name2,measure_name, wtmean_q50, unwttotal, wttotal)
                   %>%mutate(wave="GSS2014")
)
...
write.xlsx(as.data.frame(univar_agg) %>% mutate(wttotal = random_round(as.integer(univar_agg$wttotal), 1000)
           ...
           , file = "../output/desc_stats_rr_complete_medians.xlsx", sheetName = "univariate_means", row.names = FALSE,
           append = TRUE)
...
write.xlsx(as.data.frame(bivar_agg) %>% mutate(wttotal = random_round(as.integer(bivar_agg$wttotal), 1000)
           ...
           , file = "../output/desc_stats_rr_complete_medians.xlsx", sheetName = "bivariate_means", row.names = FALSE,
           append = TRUE)
