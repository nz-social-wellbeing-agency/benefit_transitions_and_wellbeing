...
/* SAS GRID. April 2019. PNH
					  dates are now being stored as sas dates in sadpit tables, not strings, 
 ...
 * Format taxyear and StartDate *;
 LENGTH TaxYear $10. ;
 TaxYear = put(ir_ir3_return_period_date,yymmdd10.) ;
 ...
 * Format taxyear and StartDate *;
 LENGTH TaxYear $10. ;
 TaxYear = put(ir_pts_return_period_date,yymmdd10.) ;
...
/* TaxYear = wff_frd_return_period_date ;*/
 ...
 TaxYear = put(wff_frd_return_period_date,yymmdd10.) ;
 /* sas grid change 2019- pnh. Dates now returned as sas dates!*/
 Enddate=wff_frd_return_period_date;
...
/* SAS Grid. Apr 2019. PNH*/
Enddate=ir_ems_return_period_date;
 ...
 ** EMS values are generally a bit higher than what is recorded in IR3/PTS/Tax Credit annual returns **;
 ...
 PROC PRINT DATA = &syslast. (obs=...) ; run ;
 ...
 *2.2 Identify where ems and annual results match *;
  ...
  ** if IR3/PTS missing then add in EMS information *;
  ...
  Period = "Annual" ;
  tempTaxYear = input(TaxYear,yymmdd10.) ;
  /* SAS Grid. Apr 2019. PNH*/;
...
DATA wff_fam_daily_rate2  ;
 SET wff_fam_daily_rate1 (WHERE = (snz_uid ne .) ) ;
  ...
  TaxYear = put(wff_fdr_return_period_date,yymmdd10.) ;
 ...
 * 3.5.3 Determine scaleF for each snz_uid taxyear and InCDudCode *;
 ...
 * 5.3.2 Loan transfers from MSD *;
 ...
 %Subset_IDIdataset2(SIDId_infile = IRD_clients
                    ...
                    ,SIDId_IDIextDt = &IMMIO_IDIexDt.
					/* PNH. April 2019. table has been moved to IDI_Adhoc*/
/*                    ,SIDId_database = idi_sandpit */
